...
Experiment.openAddExperiment = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/teacher/experiment/experiment_add'
    });
...
Experiment.openExperimentDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/teacher/experiment/experiment_update/' + Experiment.seItem.experimentId
        });
        this.layerIndex = index;
    }
};
...
Experiment.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/teacher/experiment/delete", function (data) {
            Feng.success("删除成功!");
