...
Laboratory.initColumn = function () {
    return [
        ...
        {title: 'id', field: 'laboratoryId', visible: false, align: 'center', valign: 'middle'},
            ...
            {title: '实验室是否开放', field: 'isOpen', visible: true, align: 'center', valign: 'middle', sortable: true},
            {title: '实验室是否通电', field: 'isElectrify', visible: true, align: 'center', valign: 'middle', sortable: true}
    ];
};
...
Laboratory.openAddLaboratory = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/admin/laboratory/laboratory_add'
    });
...
Laboratory.openLaboratoryDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/admin/laboratory/laboratory_update/' + Laboratory.seItem.laboratoryId
        });
        this.layerIndex = index;
    }
};
...
Laboratory.delete = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/admin/laboratory/delete", function (data) {
            Feng.success("删除成功!");
...
$(function () {
    ...
    var table = new BSTable(Laboratory.id, "/admin/laboratory/list", defaultColunms);
